...
<head>
    ...
    <style>
        ...
        h1 {{ color: #333; border-bottom: 2px solid #89B4FA; padding-bottom: 10px; }}
        ...
        .vuln-item {{ padding: 15px; margin: 10px 0; background: #fff3cd; border-left: 4px solid #ffc107; border-radius: 5px; }}
        .vuln-title {{ font-weight: bold; color: #333; margin-bottom: 5px; }}
        .vuln-severity {{ color: #dc3545; font-size: 12px; }}
        .vuln-target {{ color: #666; font-size: 12px; }}
        .vuln-desc {{ color: #666; margin-top: 5px; font-size: 13px; }}
    </style>
</head>
<body>
    <div class="container">
...
"""
    ...
    def _export_report(self):
        ...
        file_path, _ = QFileDialog.getSaveFileName(
            self,
            "导出报告",
            f"report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.{format_type.lower()}",
            f"{extensions[format_type][0]} ({extensions[format_type][1]})"
        )
        
        if file_path:
            try:
                content = self._preview_text.toPlainText()
